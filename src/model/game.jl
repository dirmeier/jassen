...
mutable struct Game
    ...
    function Game(players::Vector{Player}, seed::Integer)
        new(
            players, 
...
function play_round(game::Game)
    ...
    round = Round(game::players[0])
    for i in range(N_TRICKS_PER_ROUND)
...
function play_trick(game::Game, trick_idx::Integer, current_game_variant::String, state::Vector{Vector{Integer}})
    trick = Trick(trick_idx, game::players[1], current_game_variant)
    print("player $(game::players[1].player_idx) is opening the trick")
    for player in game.players:
       if typeof(player) == HumanPlayer
        print("human playe $(player.plyer_idx)'s turn")
